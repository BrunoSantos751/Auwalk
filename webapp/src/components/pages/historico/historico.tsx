...
const Historico: React.FC = () => {
        ...
        const fetchHistorico = async () => {
            ...
            try {
                const [resCliente, resPrestador] = await Promise.all([
                    fetch('http://http://auwalk.us-east-2.elasticbeanstalk.com/schedule/my-appointments/client', {
                        headers: { 'Authorization': `Bearer ${token}` }
                    }),
                    fetch('http://http://auwalk.us-east-2.elasticbeanstalk.com/schedule/my-appointments/provider', {
                        headers: { 'Authorization': `Bearer ${token}` }
                    })
                ]);
    ...
    const verificarAvaliacoesExistentes = async (idServicos: number[]) => {
        try {
            ...
            const promises = idServicos.map(async (idServico) => {
                try {
                    const response = await fetch(`http://http://auwalk.us-east-2.elasticbeanstalk.com/avaliacoes-prestador?idServico=${idServico}`, {
                        headers: { 'Authorization': `Bearer ${token}` }
                    });
                    if (response.ok) {
    ...
    const verificarTrajetosDisponiveis = async (agendamentos: Agendamento[]) => {
        ...
        const promises = passeios.map(async (agendamento) => {
            ...
            try {
                const response = await fetch(`http://http://auwalk.us-east-2.elasticbeanstalk.com/trajetos/simplificado/${agendamento.id_passeio}`);
                if (response.ok) {
    ...
    const simplificarTrajetoPasseio = async (idPasseio: number) => {
        try {
            ...
            const simplificarResponse = await fetch(
                `http://http://auwalk.us-east-2.elasticbeanstalk.com/trajetos/simplificar/${idPasseio}?epsilonMetros=2.5`,
                {
                    method: 'POST'
                }
            );
    ...
    const marcarComoConcluido = async (agendamento: Agendamento) => {
        try {
            ...
            const endpoint = agendamento.tipo === 'Passeio'
                ? `http://http://auwalk.us-east-2.elasticbeanstalk.com/schedule/walk/${agendamento.id}/status`
                : `http://http://auwalk.us-east-2.elasticbeanstalk.com/schedule/sitter/${agendamento.id}/status`;
    ...
    const salvarAvaliacao = async (nota: number, comentario: string) => {
        ...
        try {
            ...
            const response = await fetch('http://http://auwalk.us-east-2.elasticbeanstalk.com/avaliacoes-prestador', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
