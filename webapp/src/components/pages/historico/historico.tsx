...
const Historico: React.FC = () => {
        ...
        const fetchHistorico = async () => {
            ...
            try {
                const [resCliente, resPrestador] = await Promise.all([
                    fetch('https://api.auwalk.com.br/schedule/my-appointments/client', {
                        headers: { 'Authorization': `Bearer ${token}` }
                    }),
                    fetch('https://api.auwalk.com.br/schedule/my-appointments/provider', {
                        headers: { 'Authorization': `Bearer ${token}` }
                    })
                ]);
    ...
    const verificarAvaliacoesExistentes = async (idServicos: number[]) => {
        try {
            ...
            const promises = idServicos.map(async (idServico) => {
                try {
                    const response = await fetch(`https://api.auwalk.com.br/avaliacoes-prestador?idServico=${idServico}`, {
                        headers: { 'Authorization': `Bearer ${token}` }
                    });
                    if (response.ok) {
    ...
    const verificarTrajetosDisponiveis = async (agendamentos: Agendamento[]) => {
        ...
        const promises = passeios.map(async (agendamento) => {
            ...
            try {
                const response = await fetch(`https://api.auwalk.com.br/trajetos/simplificado/${agendamento.id_passeio}`);
                if (response.ok) {
    ...
    const simplificarTrajetoPasseio = async (idPasseio: number) => {
        try {
            ...
            const simplificarResponse = await fetch(
                `https://api.auwalk.com.br/trajetos/simplificar/${idPasseio}?epsilonMetros=2.5`,
                {
                    method: 'POST'
                }
            );
    ...
    const marcarComoConcluido = async (agendamento: Agendamento) => {
        try {
            ...
            const endpoint = agendamento.tipo === 'Passeio'
                ? `https://api.auwalk.com.br/schedule/walk/${agendamento.id}/status`
                : `https://api.auwalk.com.br/schedule/sitter/${agendamento.id}/status`;
    ...
    const salvarAvaliacao = async (nota: number, comentario: string) => {
        ...
        try {
            ...
            const response = await fetch('https://api.auwalk.com.br/avaliacoes-prestador', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
