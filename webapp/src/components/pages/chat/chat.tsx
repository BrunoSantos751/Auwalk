...
const ChatButton: React.FC = () => {
    ...
    useEffect(() => {
        if (isOpen && view === 'list') {
            const fetchChats = async () => {
                ...
                if (!token) return;
                try {
                    const response = await fetch('http://http://auwalk.us-east-2.elasticbeanstalk.com/chats', {
    ...
    useEffect(() => {
        if (activeChat) {
            const fetchMessages = async () => {
                ...
                if (!token) return;
                try {
                    const response = await fetch(`http://http://auwalk.us-east-2.elasticbeanstalk.com/mensagens/${activeChat.id_chat}`, {
                        headers: { 'Authorization': `Bearer ${token}` }
                    });
                    const result = await response.json();
    ...
    const handleSendMessage = async () => {
        ...
        try {
            const response = await fetch('http://http://auwalk.us-east-2.elasticbeanstalk.com/mensagens', {
                method: 'POST',
                headers: {
