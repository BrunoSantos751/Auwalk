...
const ChatButton: React.FC = () => {
    ...
    useEffect(() => {
        if (isOpen && view === 'list') {
            const fetchChats = async () => {
                ...
                if (!token) return;
                try {
                    const response = await fetch('https://auwalk-redirect.santosmoraes79.workers.dev/chats', {
    ...
    useEffect(() => {
        if (activeChat) {
            const fetchMessages = async () => {
                ...
                if (!token) return;
                try {
                    const response = await fetch(`https://auwalk-redirect.santosmoraes79.workers.dev/mensagens/${activeChat.id_chat}`, {
                        headers: { 'Authorization': `Bearer ${token}` }
                    });
                    const result = await response.json();
    ...
    const handleSendMessage = async () => {
        ...
        try {
            const response = await fetch('https://auwalk-redirect.santosmoraes79.workers.dev/mensagens', {
                method: 'POST',
                headers: {
